...
if yesCount > noCount {
    print("Vote has passed with \(yesCount) 'yes' votes under \(noCount) 'no' votes")
} else if yesCount < noCount {
    print("Vote has failed with \(noCount) 'no' votes under \(yesCount) 'yes' votes")
} else {
    print("Votes are equal! 'Yes' votes \(yesCount) = \(noCount) 'No' votes")
}
yesCount = 0
noCount = 0
...
func printResults(forIssue: String, withVotes: [Bool]) {
    ...
    print("\(forIssue) \(yesAgainCount) yes, \(noAgaingCount) no")
}
