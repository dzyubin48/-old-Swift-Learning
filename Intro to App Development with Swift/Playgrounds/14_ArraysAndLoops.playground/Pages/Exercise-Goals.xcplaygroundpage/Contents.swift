...
*/
// It's all my code
let dotaHoursPlayed = [1, 2, 5, 3, 0, 2, 2, 1, 1, 1, 4, 5, 3, 0, 0, 0, 0, 1, 3, 4, 0, 1, 6, 4, 0]
...
func makeAttention(hours: Int) -> String {
    if hours > 2 {
        return "Hey! You played too much that day! It was \(hours)!"
    } else if hours == 0 {
        return "Good job! You didn't played that day!"
    } else {
        return "Nice! You played a little that day! It was only \(hours)"
    }
}
...
for hours in dotaHoursPlayed {
    print(makeAttention(hours: hours))
}
